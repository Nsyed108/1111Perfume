...
if (!function_exists('get_countries')) {
    ...
    {
        return \Botble\Location\Models\Country::pluck('name', 'id')->toArray();
    }
}

if (!function_exists('getCountryDefaultId')) {
    function getCountryDefaultId()
    {
        ...
        if ($defaultCurrency && $defaultCurrency->country_id) {
            session(['country_id' => $defaultCurrency->country_id]);
            return $defaultCurrency->country_id;
        }

        return config('app.default_country_id', 1); // final fallback
    }
}