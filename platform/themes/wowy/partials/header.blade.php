                            ...
                            <div class="col-xl-4 col-lg-4">
                                <div class="header-info header-info-right">
                                        <ul>
                                            ...
                                            @if (is_plugin_active('ecommerce') && $availableCountries->count())
                                        <li>
                                            <a class="language-dropdown-active" href="#">
                                                <i class="fa fa-globe"></i>
                                                @php
                                                    $currentCountry = $availableCountries->firstWhere('id', getCountryDefaultId());
                                                @endphp

                                                @if ($currentCountry)
                                                    <img src="{{ $currentCountry->image ? asset('storage/' . $currentCountry->image) : asset('vendor/core/core/base/images/placeholder.png') }}"
                                                        alt="{{ $currentCountry->name }}" width="20" style="margin-right: 6px; border-radius: 3px;">
                                                    {{ $currentCountry->name }}
                                                @else
                                                    {{ __('Country') }}
                                                @endif
                                            ...
                                            <ul class="language-dropdown">
                                                @foreach ($availableCountries as $country)
                                                    @if ($country->id !== getCountryDefaultId())
                                                        <li>
                                                            <form action="{{ route('public.select-country') }}" method="GET" id="countryForm{{ $country->id }}">
                                                                @csrf
                                                                <input type="hidden" name="country_id" value="{{ $country->id }}">
                                                                <a href="javascript:void(0);" onclick="document.getElementById('countryForm{{ $country->id }}').submit();">
                                                                    <img src="{{ $country->image ? asset('storage/' . $country->image) : asset('vendor/core/core/base/images/placeholder.png') }}"
                                                                        alt="{{ $country->name }}" width="20" style="margin-right: 6px; border-radius: 3px;">
                                                                    {{ $country->name }}
                                                                </a>
                                                            </form>
                                                        </li>
                            ...
                            <div class="lang-curr-dropdown lang-dropdown-active">
                                <ul>
                                    @foreach ($availableCountries as $country)
                                        @if ($country->id !== getCountryDefaultId())
                                            <li>
                                                <form action="{{ route('public.select-country') }}" method="GET" id="mobileCountryForm{{ $country->id }}">
                                                    @csrf
                                                    <input type="hidden" name="country_id" value="{{ $country->id }}">
                                                    <a href="javascript:void(0);" onclick="document.getElementById('mobileCountryForm{{ $country->id }}').submit();">
                                                        <img src="{{ $country->image ? asset('storage/' . $country->image) : asset('vendor/core/core/base/images/placeholder.png') }}"
                                                            alt="{{ $country->name }}" width="20" style="margin-right: 6px; border-radius: 3px;">
                                                        {{ $country->name }}
                                                    </a>
