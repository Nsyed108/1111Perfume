...
use App\Models\ProductCountryInventory;
...
class ProductForm extends FormAbstract
{
    public function setup(): void
    {
        ...
        if ($productId) {
            $productCountryPrices = ProductCountryPrice::where('product_id', $productId)
                ->pluck('price', 'country_id')
                ->toArray();
            $inventoryModel = ProductCountryInventory::where('product_id', $product->id)
            ->first();
        }
        ...
        // Country-wise Pricing and Inventory Section
            ...
            foreach ($countries as $country) {
                ...
                if ($product && $product->id) {
                    $priceModel = ProductCountryPrice::where('product_id', $product->id)
                        ->where('country_id', $country->id)->first();
                    $inventoryModel = ProductCountryInventory::where('product_id', $product->id)
                        ->where('country_id', $country->id)->first();

                    $price = $priceModel?->price;
                    $salePrice = $priceModel?->sale_price;
                    $inventory = $inventoryModel?->quantity;
                }

                $this
                    ->add('country_price_' . $country->id, TextField::class, [
                        'label' => 'Price (' . $country->name . ')',
                        'value' => $price,
                        'attr' => ['placeholder' => 'Enter price for ' . $country->name],
                    ])
                    ->add('country_sale_price_' . $country->id, TextField::class, [
                        'label' => 'Sale Price (' . $country->name . ')',
                        'value' => $salePrice,
                        'attr' => ['placeholder' => 'Enter sale price for ' . $country->name],
                    ])
                    ->add('country_inventory_' . $country->id, TextField::class, [
                        'label' => 'Inventory (' . $country->name . ')',
                        'value' => $inventory,
                        'attr' => ['placeholder' => 'Enter inventory for ' . $country->name],
                    ]);
            }
