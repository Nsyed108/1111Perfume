...
class TrackingSettingForm extends SettingForm
{
    public function setup(): void
    {
        ...
        $this
            ...
            /** MoEngage Pixel (BEGIN) */
            ->add(
                'moengage_pixel_enabled',
                OnOffField::class,
                OnOffFieldOption::make()
                    ->label(trans('plugins/ecommerce::setting.tracking.form.enable_moengage_pixel'))
                    ->value($targetValue = EcommerceHelper::isMoengageEnabled())
            )
            ->addOpenCollapsible('moengage_pixel_enabled', '1', $targetValue)
            ->add(
                'moengage_pixel_id',
                TextField::class,
                TextFieldOption::make()
                    ->label(trans('plugins/ecommerce::setting.tracking.form.moengage_pixel_id'))
                    ->value(get_ecommerce_setting('moengage_pixel_id'))
            )
            ->add(
                'moengage_dc_id',
                TextField::class,
                TextFieldOption::make()
                    ->label(trans('plugins/ecommerce::setting.tracking.form.moengage_dc_id'))
                    ->value(get_ecommerce_setting('moengage_dc_id'))
            )
            ->add(
                'moengage_pixel_debug',
                OnOffField::class,
                OnOffFieldOption::make()
                    ->label(trans('plugins/ecommerce::setting.tracking.form.moengage_pixel_debug'))
                    ->value($targetValue = EcommerceHelper::isMoengageDebugEnabled())
            )
            ->addCloseCollapsible('moengage_pixel_enabled', '1')
            /** MoEngage Pixel (END) */
            ->add(
