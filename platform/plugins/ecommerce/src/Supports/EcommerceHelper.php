...
class EcommerceHelper
{
    ...
    {
        $byCookie = $this->getCountryByCookie();
        if ($byCookie) {
            return $byCookie;
        }
    ...
    public function getCountryByCookie(): string|null
    {
        $countryId = Cookie::get('country_id');
        $countryCode = Country::query()->where('id', request()->cookie('country_id'))->value('code');
    ...
    {
        $byCookie = $this->getCountryByCookie();
        return $byCookie ? $byCookie : Arr::first(array_filter(array_keys($this->getAvailableCountries())));
    }
    ...
    public function isMoengageEnabled(): bool
    {
        return (bool) get_ecommerce_setting('moengage_pixel_enabled', 0);
    }

    public function isMoengageDebugEnabled(): bool
    {
        return (bool) get_ecommerce_setting('moengage_pixel_debug', 0);
    }
