...
class OrderTable extends TableAbstract
{
    ...
    public function columns(): array
    {
        $columns = [
            IdColumn::make(),
            Column::make('code')
            ->title(trans('plugins/ecommerce::order.order_code'))
            ->alignStart()
            ->sortable(),
            ...
            ->title(trans('plugins/ecommerce::order.country'))
            ->alignStart()
            ->orderable(false)
            ->renderUsing(function (FormattedColumn $column) {
                $item = $column->getItem();

                return $item->address->country_name ?? '&mdash;';
            }),
            ...
            ->renderUsing(function (FormattedColumn $column) {
                ...
                return number_format($item->amount, 2). ' '.$currencySymbol;
            }),
        ];
    ...
    public function applyFilterCondition(
        ...
        switch ($key) {
            ...
            return $query->whereHas('address', function ($subQuery) use ($operator, $value) {
                if ($operator === 'like') {
                    $value = '%' . $value . '%';
                } elseif ($operator !== '=') {
                    $operator = '=';
                ...
                $subQuery->where('country', $operator, $value);
            });
